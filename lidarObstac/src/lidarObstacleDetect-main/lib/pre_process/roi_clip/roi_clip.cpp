...
{  
    private_node_handle.param("roi_x_min", roi_x_min_, -8.0);
    private_node_handle.param("roi_x_max", roi_x_max_, 18.0);
    private_node_handle.param("roi_y_min", roi_y_min_, -8.0);
    private_node_handle.param("roi_y_max", roi_y_max_, 8.0);
    private_node_handle.param("roi_z_min", roi_z_min_, -0.7);
    private_node_handle.param("roi_z_max", roi_z_max_, 2.0);
    // 转换到车辆坐标系下后，将车身点云切除，车辆坐标系中心为后轴中心０点
    // 坐标系参数不确定，暂时不转换
    private_node_handle.param("vehicle_x_min", vehicle_x_min_, 0.0);
    private_node_handle.param("vehicle_x_max", vehicle_x_max_, 0.01);
    private_node_handle.param("vehicle_y_min", vehicle_y_min_, 0.0);
    private_node_handle.param("vehicle_y_max", vehicle_y_max_, 0.01);
    private_node_handle.param("vehicle_z_min", vehicle_z_min_, 0.0);
    private_node_handle.param("vehicle_z_max", vehicle_z_max_, 0.01);
}
